    ...
    ui(new Ui::mbClientDialogDevice)
{
    ...
    uint8_t funcNum;
    //---------------- Write Function ----------------
    cmb = ui->cmbFuncWriteSingleCoil;
    funcNum = MBF_WRITE_SINGLE_COIL;
    cmb->addItem(QString("%1 - %2")
                         .arg(funcNum, 2, 10, QChar('0'))
                         .arg(mb::ModbusFunctionString(funcNum)), funcNum);

    funcNum = MBF_WRITE_MULTIPLE_COILS;
    cmb->addItem(QString("%1 - %2")
                         .arg(funcNum, 2, 10, QChar('0'))
                         .arg(mb::ModbusFunctionString(funcNum)), funcNum);
    cmb->setCurrentIndex(1); // default write function

    cmb = ui->cmbFuncWriteSingleRegister;
    funcNum = MBF_WRITE_SINGLE_REGISTER;
    cmb->addItem(QString("%1 - %2")
                         .arg(funcNum, 2, 10, QChar('0'))
                         .arg(mb::ModbusFunctionString(funcNum)), funcNum);

    funcNum = MBF_WRITE_MULTIPLE_REGISTERS;
    cmb->addItem(QString("%1 - %2")
                         .arg(funcNum, 2, 10, QChar('0'))
                         .arg(mb::ModbusFunctionString(funcNum)), funcNum);
    cmb->setCurrentIndex(1); // default write function
...
{
    const mbClientDevice::Strings &dv = mbClientDevice::Strings();
    ...
    m[prefix+dv.funcWriteSingleCoil     ] = ui->cmbFuncWriteSingleCoil      ->currentData().toInt();
    m[prefix+dv.funcWriteSingleRegister ] = ui->cmbFuncWriteSingleRegister  ->currentData().toInt();
...
void mbClientDialogDevice::setCachedSettings(const MBSETTINGS &m)
{
    ...
    if (it != end)
    {
        auto i = ui->cmbFuncWriteSingleCoil->findData(it.value().toInt());
        if (i >= 0)
            ui->cmbFuncWriteSingleCoil->setCurrentIndex(i);
    }

    it = m.find(prefix+dv.funcWriteSingleRegister);
    if (it != end)
    {
        auto i = ui->cmbFuncWriteSingleRegister->findData(it.value().toInt());
        if (i >= 0)
            ui->cmbFuncWriteSingleRegister->setCurrentIndex(i);
    }
...
void mbClientDialogDevice::fillForm(const MBSETTINGS &m)
{
    ...
    else
    {
        ...
        it = m.find(ms.funcWriteSingleCoil    );
        if (it != end)
        {
            auto i = ui->cmbFuncWriteSingleCoil->findData(it.value().toInt());
            if (i >= 0)
        ...
        it = m.find(ms.funcWriteSingleRegister);
        if (it != end)
        {
            auto i = ui->cmbFuncWriteSingleRegister->findData(it.value().toInt());
            if (i >= 0)
...
void mbClientDialogDevice::fillData(MBSETTINGS &m) const
{
    ...
    m[ms.funcWriteSingleCoil    ] = ui->cmbFuncWriteSingleCoil->currentData().toInt();
    m[ms.funcWriteSingleRegister] = ui->cmbFuncWriteSingleRegister->currentData().toInt();
